...
function utils.get_missing_languages(delimiter)
    local missing_languages = redis:smembers('oneteam:missing_languages')
...
function utils.get_inline_list(query, offset)
    ...
    local inline_help = {}
    local count = offset + 1
    table.sort(oneteam.inline_plugin_list)
    for k, v in pairs(oneteam.inline_plugin_list) do
        -- The bot API only accepts a maximum of 50 results, hence we need the offset.
        if k > offset and k < offset + 50 then
            v = v:gsub('\n', ' ')
...
function utils.send_reply(message, text, parse_mode, disable_web_page_preview, reply_markup, token)
    ...
    return oneteam.api.send_message(message, text, parse_mode, disable_web_page_preview, false, message.message_id, reply_markup, token)
end
...
function utils.toggle_user_setting(chat_id, user_id, setting)
    ...
    elseif not redis:hexists('user:' .. user_id .. ':' .. chat_id .. ':settings', tostring(setting)) then
        ...
        if setting == 'restrict messages' then
            success = oneteam.api.restrict_chat_member(chat_id, user_id, os.time(), false)
        elseif setting == 'restrict media messages' then
            success = oneteam.api.restrict_chat_member(chat_id, user_id, os.time(), nil, false)
        elseif setting == 'restrict other messages' then
            success = oneteam.api.restrict_chat_member(chat_id, user_id, os.time(), nil, nil, false)
        elseif setting == 'restrict web page previews' then
            success = oneteam.api.restrict_chat_member(chat_id, user_id, os.time(), nil, nil, nil, false)
        end
        if success then
            if setting == 'restrict messages' then
    ...
    local success = false
    if setting == 'restrict messages' then
        success = oneteam.api.restrict_chat_member(chat_id, user_id, os.time(), true)
    elseif setting == 'restrict media messages' then
        success = oneteam.api.restrict_chat_member(chat_id, user_id, os.time(), nil, true)
    elseif setting == 'restrict other messages' then
        success = oneteam.api.restrict_chat_member(chat_id, user_id, os.time(), nil, nil, true)
    elseif setting == 'restrict web page previews' then
        success = oneteam.api.restrict_chat_member(chat_id, user_id, os.time(), nil, nil, nil, true)
    end
    if success then
        if setting == 'restrict messages' then
...
function utils.input(s)
    ...
    if s:lower():match('^oneteam search %a+ for .-$') then
        return s:lower():match('^oneteam search %a+ for (.-)$')
