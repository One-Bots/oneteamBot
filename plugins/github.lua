...
function github:init()
    github.commands = oneteam.commands(self.info.username):command('github'):command('gh').table
...
function github.on_message(_, message, _, language)
    ...
    if not input then
        return oneteam.send_reply(message, github.help)
    end
    input = input:gsub('%s', '/')
    ...
    local title = '<a href="' .. jdat.html_url .. '">' .. oneteam.escape_html(jdat.full_name) .. '</a>'
    if jdat.language then
        title = title .. ' <b>|</b> ' .. jdat.language
    end
    local description = jdat.description and '\n<pre>' .. oneteam.escape_html(jdat.description) .. '</pre>\n' or '\n'
