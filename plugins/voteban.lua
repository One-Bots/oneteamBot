...
function voteban:init()
    voteban.commands = oneteam.commands(self.info.username):command('voteban').table
...
function voteban:on_message(message, configuration, language)
    ...
    local user_object = oneteam.get_user(user)
    or oneteam.get_chat(user) -- Resolve the username/ID to a user object.
    if not user_object
    then
        return oneteam.send_reply(
            message,
    ...
    elseif oneteam.get_setting(message.chat.id, 'trusted permissions voteban') and oneteam.is_trusted_user(message.chat.id, user.id)
    then
        return oneteam.send_reply(
            message,
            'I cannot start a voteban against this user because he is a trusted user and trusted users are immune to warns in this chat.'
        )
