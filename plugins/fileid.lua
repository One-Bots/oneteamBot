...
function fileid:init()
    fileid.commands = oneteam.commands(self.info.username):command('fileid'):command('fid').table
...
function fileid.on_message(_, message, _, language)
    if not message.reply or not message.reply.is_media then
        return oneteam.send_reply(message, 'You must use this command in reply to a type of media!')
    end
    local info = oneteam.unpack_file_id(message.reply.file_id, message.reply.media_type)
