...
function addtrigger:init()
    addtrigger.commands = oneteam.commands(self.info.username):command('addtrigger'):command('newtrigger'):command('addcustom').table
...
function addtrigger:on_message(message, _, language)
    ...
    if not input or not input:match('^.-\n.-$') then
        return oneteam.send_reply(message, addtrigger.help)
    end
    ...
    if is_duplicate then
        return oneteam.send_reply(message, 'That trigger already exists! To modify it, delete it first using /triggers.')
    end
    if trigger:len() > 32 then
        return oneteam.send_reply(message, 'The trigger needs to be 1-32 characters long, and alpha-numerical.')
    elseif value:len() > 512 then
        return oneteam.send_reply(message, 'The value must be no more than 512 characters long!')
    end
    redis:hset('triggers:' .. message.chat.id, trigger, value)
    return oneteam.send_reply(message, 'Successfully added that trigger! To view all of this chat\'s triggers, send /triggers.')
end
