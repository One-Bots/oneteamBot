...
function myfeds:init()
    myfeds.commands = oneteam.commands(self.info.username):command('myfeds'):command('mf').table
...
function myfeds.on_message(_, message)
    ...
    if #owned == 0 then
        ...
        for _, fed in pairs(owned) do
            ...
            table.insert(output, '<em>' .. oneteam.escape_html(title) .. '</em> <code>[' .. fed .. ']</code>')
        end
    end
    for _, fed in pairs(admins) do
        if redis:sismember(fed, message.from.id) and not oneteam.is_fed_creator(fed:match('^fedadmins:(.-)$'), message.from.id) then
