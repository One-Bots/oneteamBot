...
function administration:init()
    administration.commands = oneteam.commands(self.info.username):command('administration'):command('settings').table
...
function administration.get_initial_keyboard(chat_id, page, language)
    if not oneteam.get_setting(chat_id, 'use administration') then
        return oneteam.inline_keyboard():row(oneteam.row():callback_data_button(language['administration']['1'], 'administration:' .. chat_id .. ':toggle'))
    end
    if not page or tonumber(page) <= 1 then
        return oneteam.inline_keyboard():row(
            ...
            oneteam.row():callback_data_button(language['administration']['3'], 'antispam:' .. chat_id)
        ):row(
            oneteam.row():callback_data_button(language['administration']['4'], 'administration:' .. chat_id .. ':warnings')
        ):row(
            oneteam.row()
            ...
            oneteam.row()
            :callback_data_button(oneteam.symbols.back .. ' ' .. language['administration']['9'], 'help:settings')
            :callback_data_button(language['administration']['10'] .. ' ' .. oneteam.symbols.next, 'administration:' .. chat_id .. ':page:2')
        )
    elseif tonumber(page) == 2 then
        return oneteam.inline_keyboard():row(
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'ban not kick') and language['administration']['17'] or language['administration']['18'],
            ...
            oneteam.row()
            :callback_data_button(oneteam.symbols.back .. ' ' .. language['administration']['9'], 'administration:' .. chat_id .. ':page:1')
            :callback_data_button(language['administration']['10'] .. ' ' .. oneteam.symbols.next, 'administration:' .. chat_id .. ':page:3')
        )
    elseif tonumber(page) == 3 then
        return oneteam.inline_keyboard():row(
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'ban spamwatch users') and utf8.char(9989) or utf8.char(10060),
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'remove channel pins') and utf8.char(9989) or utf8.char(10060),
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'kick media on join') and utf8.char(9989) or utf8.char(10060),
                'administration:' .. chat_id .. ':kick_media_on_join:3'
            )
        ):row(
            oneteam.row()
            :callback_data_button(oneteam.symbols.back .. ' ' .. language['administration']['9'], 'administration:' .. chat_id .. ':page:2')
            :callback_data_button(language['administration']['10'] .. ' ' .. oneteam.symbols.next, 'administration:' .. chat_id .. ':page:4')
        )
    elseif tonumber(page) >= 4 then
        return oneteam.inline_keyboard():row(
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'remove other pins') and utf8.char(9989) or utf8.char(10060),
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'remove pasted code') and utf8.char(9989) or utf8.char(10060),
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'prevent inline bots') and utf8.char(9989) or utf8.char(10060),
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'enable plugins for admins') and utf8.char(9989) or utf8.char(10060),
            ...
            :callback_data_button(
                oneteam.get_setting(chat_id, 'kick urls on join') and utf8.char(9989) or utf8.char(10060),
                'administration:' .. chat_id .. ':kick_urls_on_join:5'
            )
        ):row(
            oneteam.row():callback_data_button(oneteam.symbols.back .. ' ' .. language['administration']['9'], 'administration:' .. chat_id .. ':page:3')
        )
    end
    return false
end
...
function administration.get_warnings(chat_id, language)
    ...
    local current = oneteam.get_setting(chat_id, 'max warnings') or configuration.administration.warnings.default
...
function administration.del_chat(message, language)
    ...
    for _, v in pairs(redis:smembers('oneteam:configuration:chats')) do
        if not v or not json.decode(v).link or not json.decode(v).title then
            return
        elseif json.decode(v).title == title then
            redis:srem('oneteam:configuration:chats', v)
            return oneteam.send_reply(message, string.format(language['administration']['29'], title))
        end
    end
    return oneteam.send_reply(message, string.format(language['administration']['30'], title))
end
...
function administration.on_callback_query(_, callback_query, message, _, language)
    ...
    elseif not oneteam.is_group_admin(callback_query.data:match('^(%-%d+)'), callback_query.from.id) then
        return oneteam.answer_callback_query(callback_query.id, language['administration']['31'])
    end
    local keyboard
    ...
    elseif callback_query.data:match('^%-%d+:max_warnings:.-$') then
        ...
        if tonumber(max_warnings) > configuration.administration.warnings.maximum then
            return oneteam.answer_callback_query(callback_query.id, string.format(language['administration']['36'], configuration.administration.warnings.maximum))
        elseif tonumber(max_warnings) < configuration.administration.warnings.minimum then
            return oneteam.answer_callback_query(callback_query.id, string.format(language['administration']['37'], configuration.administration.warnings.minimum))
    ...
    elseif callback_query.data:match('^%-%d+:toggle$') then
        ...
        oneteam.toggle_setting(chat_id, 'use administration')
    ...
    elseif callback_query.data:match('^%-%d+:rtl:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'antirtl')
    ...
    elseif callback_query.data:match('^%-%d+:welcome_message:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'welcome message')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    ...
    elseif callback_query.data:match('^%-%d+:force_group_language:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'force group language')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    ...
    elseif callback_query.data:match('^%-%d+:settings_in_group:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'settings in group')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:kick_media_on_join:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'kick media on join')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:delete_reply_on_action:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'delete reply on action')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:require_captcha:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'require captcha')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:ban_spamwatch_users:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'ban spamwatch users')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:remove_channel_pins:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'remove channel pins')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:remove_other_pins:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'remove other pins')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:remove_pasted_code:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'remove pasted code')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:prevent_inline_bots:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'prevent inline bots')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:enable_plugins_for_admins:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'disable plugins for all')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    elseif callback_query.data:match('^%-%d+:kick_urls_on_join:%d*$') then
        ...
        oneteam.toggle_setting(chat_id, 'kick urls on join')
        keyboard = administration.get_initial_keyboard(chat_id, page, language)
    ...
    elseif callback_query.data == 'dismiss_disabled_message' then
        ...
        return oneteam.answer_callback_query(callback_query.id, language['administration']['39'], true)
    else return oneteam.answer_callback_query(callback_query.id) end
    oneteam.answer_callback_query(callback_query.id)
    return oneteam.edit_message_reply_markup(message.chat.id, message.message_id, nil, keyboard)
end
...
function administration:on_message(message, _, language)
    if message.chat.type == 'private' then
        ...
        if input then
            ...
            if resolved and oneteam.is_group_admin(resolved.result.id, message.from.id) then
                message.chat = resolved.result
                message.message_id = nil
            elseif resolved then
                return oneteam.send_reply(message, language['administration']['40'])
            else
                return oneteam.send_reply(message, language['administration']['41'])
            end
        else
            return oneteam.send_reply(message, language['administration']['42'])
        end
    end
    if oneteam.is_group_admin(message.chat.id, message.from.id) then
        ...
        local output = string.format(language['administration']['43'], oneteam.escape_html(message.chat.title))
        local success = oneteam.send_message(recipient, output, 'html', true, false, nil, keyboard)
        if not success and recipient == message.from.id then
            return oneteam.send_reply(message, string.format(language['administration']['44'], self.info.username:lower()), true)
        elseif recipient == message.from.id then
            return oneteam.send_reply(message, language['administration']['45'])
        end
        return success
    end
    return false
