...
function plugins:init()
    plugins.commands = oneteam.commands(self.info.username):command('plugins').table
...
function plugins:on_callback_query(callback_query, message, configuration)
    ...
    if (
        oneteam.get_chat(chat) and oneteam.get_chat(chat).result.type ~= 'private'
    ) and not oneteam.is_group_admin(
        chat,
    ...
    oneteam.edit_message_reply_markup(message.chat.id, message.message_id, nil, keyboard)
    plugins.refresh(self, chat)
    return oneteam.answer_callback_query(callback_query.id, toggle_status)
end

function plugins:on_message(message, configuration)
    if message.chat.type ~= 'private' and not oneteam.is_group_admin(
        message.chat.id,
        message.from.id
    ) then
    ...
    local success = oneteam.send_message(
        ...
        string.format(
            ...
            message.chat.title and oneteam.escape_html(message.chat.title) or oneteam.escape_html(message.chat.first_name)
        ),
        'html',
        true,
        false,
