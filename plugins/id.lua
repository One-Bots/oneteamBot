...
function id:init()
    id.commands = oneteam.commands(self.info.username):command('id'):command('user'):command('whoami').table
...
function id.resolve_chat(input, language, send_chat_action, current_group, show_user_stats, api_mode)
    ...
    elseif send_chat_action and current_group then
        oneteam.send_chat_action(current_group, 'typing')
    end
    local success = oneteam.get_user(input, false, true) or oneteam.get_chat(input)
    ...
    if success.result.type == 'private' then
        ...
        if nickname then
            nickname = ', AKA <em>' .. oneteam.escape_html(nickname) .. '</em>'
        else
            nickname = ''
        end
        table.insert(output, oneteam.get_formatted_user(success.result.id, name, 'html') .. nickname .. ' <code>[' .. success.result.id .. ']</code>')
        ...
        if success.result.bio and success.result.bio ~= '' then
            table.insert(output, '<b>Bio:</b> <code>' .. oneteam.escape_html(success.result.bio) .. '</code>')
        end
        ...
        if current_group and show_user_stats then
            local chat_member = oneteam.get_chat_member(current_group, success.result.id)
        ...
        table.insert(output, oneteam.escape_html(success.result.title) .. ' <code>[' .. success.result.id .. '</code> (' .. success.result.type .. ')')
        if success.result.username then
            table.insert(output, '@' .. success.result.username)
        end
        if current_group ~= success.result.id then
            if success.result.description and success.result.description ~= '' then
                table.insert(output, '<b>Description:</b> <code>' .. oneteam.escape_html(success.result.description) .. '</code>')
            end
        end
    end
    return output
...
function id.on_inline_query(_, inline_query, _, language)
    local input = oneteam.input(inline_query.query) or inline_query.from.id
    ...
    return oneteam.answer_inline_query(
        ...
        :input_message_content(
            oneteam.input_text_message_content(output, 'html')
        )
    )
end
...
function id.on_message(_, message, _, language)
   ...
   local is_reply, is_reply_chat, old_chat_object = false, false, message.chat
    ...
    oneteam.send_chat_action(old_chat_object.id)
    local has_input = oneteam.input(message.text)
    local input = oneteam.input(message.text) or message.from.id
