...
function transcribe:init()
    transcribe.commands = oneteam.commands(self.info.username):command('transcribe'):command('tts').table
...
function transcribe.on_new_message(_, message)
 ...
 function transcribe.on_message(_, message)
    if not message.reply then
        return oneteam.send_reply(message, transcribe.help)
    elseif not message.reply.voice then
        return oneteam.send_reply(message, 'You can only use this message in reply to voice messages!')
    elseif not transcribe.is_valid(message) then
        return oneteam.send_reply(message, 'The voice message must meet the following conditions:\n1. It must be 20 seconds or less in length\n2. It must be 20MB or less in file size\n3. It must be of the audio/ogg MIME type')
    end
