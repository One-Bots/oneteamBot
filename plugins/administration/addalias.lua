...
function addalias:init(configuration)
    addalias.commands = oneteam.commands(self.info.username):command('addalias'):command('setalias').table
...
function addalias:on_message(message, configuration, language)
    ...
    if not input or not input:match('^/[%w_]+ /[%w_]+$') then
        return oneteam.send_reply(message, 'Please use the format /addalias /command /alias.')
    end
    local existing, alias = input:match('^/([%w_]+) /([%w_]+)$')
    local length = alias:len()
    if length < addalias.limits.length.min then
        return oneteam.send_reply(message, 'The alias must be a minimum of ' .. addalias.limits.length.min .. ' characters long!')
    elseif length > addalias.limits.length.max then
        return oneteam.send_reply(message, 'The alias must not be longer than ' .. addalias.limits.length.max .. ' characters long!')
    end
    local total = redis:hgetall('chat:' .. message.chat.id .. ':aliases')
    if #total >= addalias.limits.total then
        return oneteam.send_reply(message, 'You\'ve can\'t add anymore than ' .. addalias.limits.total .. ' aliases, please use /aliases and delete some to add more!')
    end
    ...
    for _, command in pairs(all) do
        if ('/' .. alias):match(command) then
            return oneteam.send_reply(message, 'You can\'t set this as a command alias, because this is already bound to a plugin!')
        elseif ('/' .. existing):match(command) then
            is_valid = true
        end
    end
    if not is_valid then
        return oneteam.send_reply(message, 'I couldn\'t set that as an alias because the command you specified doesn\'t exist!')
    end
    local exists = redis:hget('chat:' .. message.chat.id .. ':aliases', alias)
    if exists then
        return oneteam.send_reply(message, 'I can\'t set /' .. alias .. ' as an alias because it\'s already bound to the following command: /' .. exists)
    end
    redis:hset('chat:' .. message.chat.id .. ':aliases', alias, existing)
    return oneteam.send_reply(message, 'Success! You can now trigger the command /' .. existing .. ' using the alias /' .. alias .. '!')
end
