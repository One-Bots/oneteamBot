...
function wordfilter:on_new_message(message)
    if message.chat.type == 'supergroup' and oneteam.get_setting(message.chat.id, 'word filter') and not oneteam.is_group_admin(message.chat.id, message.from.id) then
                ...
                if base:match('^' .. v:lower() .. '$') or base:match('^' .. v:lower() .. ' ') or base:match(' ' .. v:lower() .. ' ') or base:match(' ' .. v:lower() .. '$') then
                    ...
                    if success then
                        if oneteam.get_setting(message.chat.id, 'log administrative actions') then
                            ...
                            oneteam.send_message(log_chat, string.format('<pre>%s [%s] has kicked %s [%s] from %s [%s] for sending one or more prohibited words.</pre>', oneteam.escape_html(self.info.first_name), self.info.id, oneteam.escape_html(message.from.first_name), message.from.id, oneteam.escape_html(message.chat.title), message.chat.id), 'html')
                        end
...
function wordfilter:on_message(message, configuration, language)
    ...
    local words = redis:smembers('word_filter:' .. message.chat.id)
    if #words < 1 then
        return oneteam.send_reply(message, 'There are no words filtered in this chat. To add words to the filter, use /filter <word(s)>.')
    end
    return oneteam.send_message(message.chat.id, 'Filtered words: ' .. table.concat(words, ', '))
end
