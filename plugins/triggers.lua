    ...
    for trigger, value in pairs(matches) do
        if message.text:lower():match(trigger:lower()) then
            ...
            if value:match('%b{}') then
                ...
                if message.chat.type == 'supergroup' then
                    ...
                    if value:match('{usercount}') then
                        user_count = oneteam.get_chat_members_count(message.chat.id).result
                    end
            ...
            if not message.is_edited then
                local success = oneteam.send_message(message.chat.id, value)
                if success then
                ...
                if message_id then
                    return oneteam.edit_message_text(message.chat.id, message_id, value)
                end
            end
        end
    end
...
function triggers.get_keyboard(chat_id, page, columns, per_page)
    ...
    if page_count > 1 then
        table.insert(keyboard.inline_keyboard, {{
            ['text'] = oneteam.symbols.back .. ' Previous',
            ['callback_data'] = string.format(
                'triggers:%s:page:%s',
                chat_id,
                page - 1
            ...
            ['text'] = 'Next ' .. oneteam.symbols.next,
            ['callback_data'] = string.format(
                'triggers:%s:page:%s',
                chat_id,
                page + 1
...
function triggers.on_callback_query(_, callback_query, message)
    ...
    if not oneteam.is_group_admin(chat_id, callback_query.from.id) then
        return oneteam.answer_callback_query(callback_query.id, 'You are not allowed to use this!')
    end
    ...
    elseif callback_type == 'delete' then
        local all = redis:hgetall('triggers:' .. chat_id)
        if not next(all) then
            return oneteam.answer_callback_query(callback_query.id, 'All triggers saved for this chat have already been deleted!')
        end
        local selected = all[page]
        if not selected then
            return oneteam.answer_callback_query(callback_query.id, 'This trigger no longer exists!')
        end
        redis:hdel('triggers:' .. chat_id, page)
        return oneteam.answer_callback_query(callback_query.id, 'That trigger has been deleted from my database!')
    ...
    value = oneteam.escape_html(value)
    local success = oneteam.edit_message_text(
        message.chat.id,
        message.message_id,
...
function triggers.on_message(_, message)
    ...
    if not keyboard then
        return oneteam.send_reply(message, 'This chat does\'t have any triggers saved in my database! Use /addtrigger <trigger> <value> to add one!')
    end
    return oneteam.send_message(message.chat.id, 'Please select a trigger:', nil, true, false, nil, keyboard)
end
